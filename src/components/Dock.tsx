...
function DockIcon({ children, className = "" }: DockIconProps) {
  return (
    <div className={`flex items-center justify-center text-silver ${className}`}>{children}</div>
  );
}
...
export default function Dock({
  ...
  return (
    <motion.div
      ...
      className="mx-2 flex max-w-full items-center fixed bottom-0 left-0 right-0 "
    >
      <motion.div
        onMouseMove={({ pageX }) => {
          isHovered.set(1);
        ...
        className={`${className} absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-center w-fit gap-4 rounded-2xl px-4`}
